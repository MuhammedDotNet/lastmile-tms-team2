...
export type CancelRouteDialogProps = {
  open: boolean;
      ...
      >
        <div className="relative border-b border-destructive/15 bg-linear-to-br from-destructive/[0.07] via-transparent to-amber-500/4 px-6 pt-6 pb-5">
          <div className="flex gap-4">
            ...
            <div className="min-w-0 flex-1 space-y-1.5 pt-0.5">
              <h2
                id="cancel-route-title"
                className="text-lg font-semibold leading-tight tracking-tight"
              >
                Cancel this route?
              </h2>
              <p className="text-sm leading-relaxed text-muted-foreground">
                This removes the route from the active dispatch workflow, frees
                its vehicle for other assignments, and returns staged parcels to
                sorted.
              </p>
            </div>
          </div>
        ...
        <div className="space-y-3 px-6 py-5">
          <p className="text-sm text-muted-foreground">
            The route record will stay visible in history with status{" "}
            <span className="font-medium text-foreground">Cancelled</span>.
          </p>
          <label htmlFor="cancel-route-reason" className="block text-sm font-medium">
            Cancellation reason
          </label>
          <textarea
            id="cancel-route-reason"
            value={reason}
            onChange={(event) => {
            ...
            placeholder="Explain why this route should be cancelled."
